...
#define RATE 100.0f
#define G 245.25f // acceleration due to gravity
...
void Game::init_maze() {
	for (int i = 0; i < 32; i++) {
		for (int j = 0; j < 32; j++) {
			...
			if (maze[i][j]) MatrixController::update_pixel(i, j, 1, 0, 0);
		}
	}
}
...
bool edge_check(float prev_x, float prev_y, float curr_x, float curr_y, int x_dir, int y_dir) {
	return maze[round(prev_x + x_dir)][round(prev_y + y_dir)] && round(prev_x + x_dir) == round(curr_x) && round(prev_y + y_dir) == round(curr_y);
}
...
void Game::update_ball() {
	...
	if (edge_check(prev_x, prev_y, ball.pos_x, ball.pos_y, 1, 0)) {
		...
		if (maze[round(prev_x) + 1][round(prev_y)]) {
			if (ball.vel_x > 0) ball.vel_x = 0;
			ball.pos_x = prev_x;
		}
		if (maze[round(prev_x)][round(prev_y) + 1]) {
			if (ball.vel_y > 0) ball.vel_y = 0;
			ball.pos_y = prev_y;
		}
		if (!maze[round(prev_x) + 1][round(prev_y)] && !maze[round(prev_x)][round(prev_y) + 1]) {
